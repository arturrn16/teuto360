...
import { SolicitacaoTransporteRota } from "@/types/solicitacoes";
import { Textarea } from "@/components/ui/textarea";
import { Label } from "@/components/ui/label";
import { AdminCommentField } from "./AdminCommentField";

interface SolicitacaoTransporteRotaViewProps {
  solicitacao: SolicitacaoTransporteRota;
  onStatusChange: () => void;
}

export function SolicitacaoTransporteRotaView({ 
  solicitacao, 
  onStatusChange 
}: SolicitacaoTransporteRotaViewProps) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <h3 className="text-sm font-medium text-muted-foreground">Nome do Colaborador</h3>
            <p>{solicitacao.colaborador_nome}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 className="text-sm font-medium text-muted-foreground">Período Início</h3>
            <p>{new Date(solicitacao.periodo_inicio).toLocaleDateString()}</p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-muted-foreground">Período Fim</h3>
            <p>{new Date(solicitacao.periodo_fim).toLocaleDateString()}</p>
          </div>
        </div>
