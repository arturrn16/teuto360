...
const Admin = () => {
  ...
  const fetchSolicitacoes = async () => {
    try {
      ...
      } else if (dataRefeicao) {
        // Ensure colaboradores is always treated as a string array
        const processedData = dataRefeicao.map(item => ({
          ...item,
          colaboradores: Array.isArray(item.colaboradores) ? item.colaboradores : [],
          // Also handle matriculas if present
          matriculas: item.matriculas && Array.isArray(item.matriculas) ? item.matriculas : []
        }));
        setSolicitacoesRefeicao(processedData);
      }
