    ...
    try {
      ...
      if (refeicoes) {
        refeicoes.forEach(refeicao => {
          ...
          const colaboradoresCount = refeicao.colaboradores?.length || 0;
          
          // Incrementa a contagem por tipo
          contagem[tipo] = (contagem[tipo] || 0) + colaboradoresCount;
          
          // Incrementa a contagem por dia e tipo
          if (!dataByDay[data]) {
            dataByDay[data] = {};
          }
          dataByDay[data][tipo] = (dataByDay[data][tipo] || 0) + colaboradoresCount;
      ...
      const chartDataArray = Object.keys(contagem).map(key => ({
        name: key,
        value: contagem[key]
      }));
      
      // Dados para o gráfico por dia
      const dailyChartData = Object.keys(dataByDay).map(date => ({
        date,
        ...dataByDay[date]
      }));
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Relatórios de Refeição</h1>
        <Button 
          variant="ghost" 
          onClick={handleBack} 
...
export default RelatoriosRefeicao;
